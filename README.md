<div align="center">
  <h1>Hi 👋, I'm Rasel Howlader</h1>
<a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.demolab.com?font=Fira+Code&duration=3500&pause=1000&color=02A900&center=true&vCenter=true&width=435&lines=Owner+of+ARN+TECH;Founder+of+ThemeIM;Founder+%26+System+Admin+of+ARN+HOST" alt="Typing SVG" /></a>

  <p><strong>Leading the way in DevOps and Product Manager.</strong></p>

  <a href="https://ko-fi.com/community_scripts">
    <img src="https://img.shields.io/badge/Install_the_app-ThemeIM-F16061?style=for-the-badge&logo=ko-fi&logoColor=white" alt="Install Shopify App" />
  </a>
</div>

---

### 🚀 Top Project
<div align="center">
  <a href="https://github.com/raselhowlader/UniAI">
    Uni AI
  </a>
</div>

---

### 💡 About Me & Tech Stack
<div align="center">
  <table>
    <tr>
      <td width="50%" valign="top">
        <strong>Role & Mission</strong>
        <ul>
          <li>🛠️ <b>Owner</b>: <a href="https://arntechbd.com">ARN TECH</a></li>
          <li>🚀 <b>Founder</b>: <a href="https://themeim.com">ThemeIM</a></li>
          <li>🏠 <b>Passion</b>: DevOps & System Admin</li>
        </ul>
      </td>
      <td width="50%" valign="top">
        <strong>Tech Stack</strong>
        <ul>
          <li>💻 <b>Languages</b>: Java, Python, Go, Rust, TS, Svelte</li>
          <li>🐳 <b>Core</b>: Virtualization, Docker, Home Assistant</li>
          <li>🐧 <b>OS</b>: Linux (Debian/Ubuntu)</li>
        </ul>
      </td>
    </tr>
  </table>
</div>

---

### 🌟 Essential Apps & Recommendations
*Curated selection of tools I use and recommend:*
<div align="center">
  <table width="100%">
    <tr>
      <td width="33.3%" align="center">🏠 <b>Speed Cart</b></td>
      <td width="33.3%" align="center">⚡ <b>Charge UP</b></td>
      <td width="33.3%" align="center">🖥️ <b>Assurify</b></td>
    </tr>
    <tr>
      <td align="center">📦 <b>ARN HOST</b></td>
      <td align="center">🛡️ <b>WordPress</b></td>
      <td align="center">🎨 <b>Shopify</b></td>
    </tr>
    <tr>
      <td align="center">📸 <b>Asterisk</b></td>
      <td align="center">🍴 <b>FreeSWITCH</b></td>
      <td align="center">🎵 <b>SIP Server</b></td>
    </tr>
  </table>
</div>

---


### 📊 GitHub Activity

<div align="center">
  <img height="150" src="https://streak-stats.demolab.com?user=raselhowlader&theme=tokyonight&fire=7c4dff" alt="Streak" />
</div>

---

<div align="center">
  <img src="https://komarev.com/ghpvc/?username=raselhowlader&color=7c4dff&style=flat-square&label=PROFILE+VIEWS" alt="Views" />
</div>






